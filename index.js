...
// Resolve a short WAQAR-WRITES~<token> reference to its KnightBot! payload
// by fetching from the session generator service.
async function resolveShortSession(shortId) {
  const token = shortId.split('~')[1];
  if (!token) throw new Error('Invalid WAQAR-WRITES session id');

  const base = (process.env.SESSION_GEN_URL || 'http://localhost:80/session').replace(/\/$/, '');
  const url = `${base}/s/${encodeURIComponent(token)}`;

  const res = await fetch(url);
  if (!res.ok) throw new Error(`Session lookup failed: HTTP ${res.status}`);
  const text = (await res.text()).trim();
  if (!text.startsWith('KnightBot!')) throw new Error('Resolved payload is not a valid session');
  return text;
}
...
async function startBot() {
  ...
  // STRICT: bot accepts ONLY sessions generated by the WAQAR WRITES Session Generator.
  // Any other format (raw KnightBot!, plain text, base64, etc.) is rejected.
  let sessionFromGenerator = false;

  // If creds.json already exists locally (first connect already happened, or this is a reconnect),
  // skip the SESSION_ID resolution entirely and reuse the on-disk session. This prevents reconnect
  // failures because config.sessionID gets mutated to the resolved KnightBot! payload after the
  // first run, which would no longer match the WAQAR-WRITES~ prefix check.
  if (fs.existsSync(sessionFile) && fs.statSync(sessionFile).size > 0) {
    console.log('📡 Session : ✅ Using existing local session (skipping resolve)');
  } else if (config.sessionID && config.sessionID.startsWith('WAQAR-WRITES~')) {
    try {
      console.log('📡 Session : Resolving WAQAR-WRITES short reference...');
      config.sessionID = await resolveShortSession(config.sessionID);
      console.log('📡 Session : ✅ Short reference resolved');
      sessionFromGenerator = true;
    } catch (e) {
      console.error('📡 Session : ❌ Failed to resolve short reference:', e.message);
      console.error('📡 Session : Bot will NOT start. Generate a new session at the WAQAR WRITES Session Generator.');
      process.exit(1);
    }
  } else if (config.sessionID) {
    console.error('📡 Session : ❌ REJECTED - SESSION_ID must start with "WAQAR-WRITES~"');
    console.error('📡 Session : Only sessions generated by the WAQAR WRITES Session Generator are accepted.');
    console.error('📡 Session : Visit the Session Generator and link your device to obtain a valid session.');
    process.exit(1);
  } else {
    console.error('📡 Session : ❌ No SESSION_ID provided.');
    console.error('📡 Session : Generate one at the WAQAR WRITES Session Generator and set it as SESSION_ID env var.');
    process.exit(1);
  }

  // Process the resolved KnightBot! payload (only reachable when session is from our generator)
  if (sessionFromGenerator && config.sessionID && config.sessionID.startsWith('KnightBot!')) {
    try {
...
// Start the bot
console.log('🚀 Starting 𝑾𝑨𝑸𝑨𝑹 𝑾𝑹𝑰𝑻𝑬𝑺 𝑴𝑫 Bot...\n');
console.log(`📦 Bot Name: ${config.botName}`);
console.log(`⚡ Prefix: ${config.prefix}`);
const ownerNames = Array.isArray(config.ownerName) ? config.ownerName.join(',') : config.ownerName;
console.log(`👑 Owner: ${ownerNames}\n`);

...
// ─── ANTIDELETE: handle a revoke event by resending the cached message ───
const handleRevoke = async (sock, revokeMsg, deletedKey) => {
  try {
    const chatJid = revokeMsg.key.remoteJid;
    const isGroup = chatJid.endsWith('@g.us');
    // Only act in groups (where antidelete makes sense as an admin feature)
    if (!isGroup) return;

    const settings = database.getGroupSettings(chatJid);
    if (!settings.antidelete) return;

    const cached = messageStore.get(chatJid, deletedKey.id);
    if (!cached) return; // we never saw the original (bot was offline / cache miss)

    // Don't recover messages deleted by the original sender's request if it was an admin/owner
    const deletedBy = revokeMsg.key.participant || revokeMsg.key.remoteJid;
    if (isOwner(deletedBy)) { messageStore.remove(chatJid, deletedKey.id); return; }

    const dest = settings.antideleteDest || 'here';
    let target = chatJid;
    if (dest === 'inbox') {
      const ownerNum = (Array.isArray(config.ownerNumber) ? config.ownerNumber[0] : config.ownerNumber);
      if (ownerNum) target = `${String(ownerNum).replace(/\D/g, '')}@s.whatsapp.net`;
    }

    const senderTag = cached.sender ? `@${cached.sender.split('@')[0]}` : (cached.pushName || 'someone');
    const header = `*ANTIDELETE*\nDeleted by: ${senderTag}\nIn: ${isGroup ? 'group' : 'chat'}\n──────────────`;
    ...
    // Media types — download & re-upload (cannot forward by reference once revoked)
    const mediaTypes = [
      ['imageMessage',    'image'],
      ['videoMessage',    'video'],
      ['audioMessage',    'audio'],
    ...
    for (const [key, kind] of mediaTypes) {
      if (content[key]) {
        try {
          ...
          const payload = { caption: `${header}${caption}`, mentions };
          if (kind === 'image')    payload.image    = buffer;
          if (kind === 'video')    { payload.video    = buffer; payload.mimetype = content[key].mimetype || 'video/mp4'; }
          if (kind === 'audio')    { payload.audio    = buffer; payload.mimetype = content[key].mimetype || 'audio/ogg; codecs=opus'; payload.ptt = !!content[key].ptt; delete payload.caption; }
          if (kind === 'sticker')  { payload.sticker  = buffer; delete payload.caption; }
          if (kind === 'document') { payload.document = buffer; payload.mimetype = content[key].mimetype || 'application/octet-stream'; payload.fileName = content[key].fileName || 'file'; }
          // header as a separate text msg for sticker/audio (no caption support)
          if (kind === 'sticker' || kind === 'audio') {
            await sock.sendMessage(target, { text: header, mentions });
          }
...
const handleMessage = async (sock, msg) => {
  try {
    ...
    // ── ANTIDELETE: cache + revoke detection ────────────────────────────────
    try {
      // Detect message revoke (someone deleted a message)
      const proto = msg.message.protocolMessage;
      if (proto && proto.type === 0 && proto.key) {
        await handleRevoke(sock, msg, proto.key).catch(() => {});
        return;
      }
      // Cache every incoming message (skip bot's own & system)
      messageStore.save(msg);
    } catch { /* never break the bot */ }
    // ────────────────────────────────────────────────────────────────────────

    
    // Auto-React System
    try {
      // Use cached config — owner commands that mutate settings invalidate the cache themselves
      const config = require('./config');
    ...
    // Chatbot: respond per active mode (groups / dm / inbox). Non-prefix messages only.
    if (!body.startsWith(config.prefix) && !msg.key.fromMe) {
      try {
        const chatbot = require('./commands/admin/chatbot');
        await chatbot.handleChatbotResponse(sock, from, msg, body, sender);
      } catch (cbErr) {
        console.error('Chatbot hook error:', cbErr.message);
      }
    }

...
try { ytdl = require('@distube/ytdl-core'); } catch (_) {
  try { ytdl = require('ytdl-core'); } catch (_) { ytdl = null; }
}
...
// ── ytdl-core fallback (used when yt-dlp binary is missing) ────────────────
...
async function ytdlAudio(url) {
  if (!ytdl) throw new Error('No YouTube downloader available (yt-dlp + ytdl-core both missing)');
...
async function ytdlVideo(url, quality = '360') {
  if (!ytdl) throw new Error('No YouTube downloader available (yt-dlp + ytdl-core both missing)');
  const stream = ytdl(url, {
...
function isEnoent(err) {
  ...
  return /ENOENT|not found|exited 127|command not found/i.test(m);
}
// ───────────────────────────────────────────────────────────────────────────

// Download YouTube audio as Buffer (mp3 via yt-dlp, or m4a via ytdl-core fallback).
async function ytAudio(urlOrQuery) {
  ...
  try {
    ...
    if (isEnoent(err)) {
      // yt-dlp not installed (e.g. local dev) — fall back to ytdl-core
      const buf = await ytdlAudio(url);
      return { buffer: buf, title: meta?.title || 'audio', thumbnail: meta?.thumbnail, duration: meta?.duration, url };
    }
    throw err;
...
// Download YouTube video as Buffer (mp4 via yt-dlp, or ytdl-core fallback).
async function ytVideo(urlOrQuery, quality = '360') {
  ...
  try {
    ...
    if (isEnoent(err)) {
      const buf = await ytdlVideo(url, quality);
      return { buffer: buf, title: meta?.title || 'video', thumbnail: meta?.thumbnail, duration: meta?.duration, url };
    }
    throw err;
  } finally { try { fs.unlinkSync(out); } catch {} }
